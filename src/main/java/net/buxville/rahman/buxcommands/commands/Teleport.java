...
public class Teleport {
			...
			List<Player> ts = Bukkit.getServer().matchPlayer(args[0]);
	          if (ts.size() > 0) {
	            ...
	            if ((tg != null) && (p.isOnline()) && (!tg.equals(p))) {
	              ...
	              tprequests.put(p.getUniqueId(), tg.getUniqueId());
	            } else {
	...
	public static void TPCto(Player p, String[] args) {
		if (args.length == 0) {
			p.sendMessage(ChatColor.RED + "To use this command correctly, use /tpcto [player].");
		}
		if (args.length == 1) {
	        @SuppressWarnings("deprecation")
			List<Player> ts = Bukkit.getServer().matchPlayer(args[0]);
	          if (ts.size() > 0) {
	            ...
	            if ((tg != null) && (p.isOnline()) && (!tg.equals(p))) {
	              tg.sendMessage(ChatColor.WHITE + p.getName() + ChatColor.DARK_GREEN + 
	                " would like to teleport to your current location. Type " + ChatColor.YELLOW + "/acc" + ChatColor.DARK_GREEN + 
	                " or " + ChatColor.YELLOW + "/den" + ChatColor.DARK_GREEN + ".");
	              p.sendMessage(ChatColor.DARK_GREEN + "You have sent a teleport request to " + ChatColor.WHITE + tg.getName() + ChatColor.DARK_GREEN + 
	                ".");
